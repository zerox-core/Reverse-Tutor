...
| # | 事项 | 我的建议 |
|---|---|---|
| D1 | G1+G2 冻结层追溯确认（S4–S7 持久化 + V1-006 core/llm） | 确认，并在 capability-requests 补一条追认记录 |
| D2 | 是否建 API ≤ 34 AVD（仅 Room migration instrumentation 需要） | 不急建，标 blocked 即可，后续有迁移风险再建 |
| D3 | 是否补 LLM 生成开场轮（现为确定性模板） | 暂不补——模板是现成产品设计且零 LLM 成本；若你要 engine.py 同等表现可作小任务 |
| D4 | Codex 26 份未提交文件处置 | 建议 Codex 自行收口提交（内容属 V1-006 批次，我没有完整上下文） |
