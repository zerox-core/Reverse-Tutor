...
- 28 个 P0 替换阻塞项中，仅 4 项已验证（`verified`），24 项仍为 `blocked`
...
| LEG-044 | 应用诊断/关于 | ✅ | ✅ | ✅ P3-004 | ✅ | verified | `FormalDiagnosticsScreens.kt`，诊断策略 JVM 测试 |
...
| LEG-038 | 头像系统 | ✅ | ✅ | not_run | ✅ | blocked | per-session 头像 choose/clear/hide 未实现 |
...
| LEG-041 | 键盘处理 | ✅ | ✅ | not_run | ✅ | blocked | 更广泛设备矩阵待 P6 |
| LEG-016 | 流式与队列 | ✅ | ✅ | not_run | ✅ | blocked | P6 设备矩阵待补 |
| LEG-017 | 后台回复 | ✅ | ✅ | not_run | ✅ | blocked | 设备矩阵、真实 Provider 调用待补 |
| LEG-042 | 通知系统 | ✅ | ✅ | not_run | ✅ | blocked | 设备矩阵、安全文案设备验证待补 |
| LEG-020 | 上下文 Hub | ✅ | ✅ | not_run | ✅ | blocked | 设备矩阵待补（历史 4 tests passed） |
| LEG-021 | 证据跳转 | ✅ | ✅ | not_run | ✅ | blocked | 设备矩阵待补 |
| LEG-022 | 返回 Chat | ✅ | ✅ | not_run | ✅ | blocked | 设备矩阵待补 |
| LEG-024 | 图谱 Canvas | ✅ | ✅ | not_run | ✅ | blocked | 设备矩阵待补（历史 5 tests passed） |
| LEG-025 | 图谱交互 | ✅ | ✅ | not_run | ✅ | blocked | 设备矩阵待补 |
| LEG-026 | 记忆锚点 | ✅ | ✅ | not_run | ✅ | blocked | 设备矩阵待补（历史 1 test passed） |
| LEG-027 | 记忆笔记 | ✅ | ✅ | not_run | ✅ | blocked | 笔记管理 UI、设备矩阵待补 |
| LEG-030 | 来源管理 | ✅ | ✅ | not_run | ✅ | blocked | 事务加固、复杂解析器、设备矩阵待补 |
| LEG-031 | 来源导入 | ✅ | ✅ | not_run | ✅ | blocked | 设备矩阵待补（历史 1 test passed） |

> 注：LEG-032（来源预览/解析状态）归入 LEG-030/031 的设备矩阵覆盖范围。
...
| 条件 | 状态 | 说明 |
|---|---|---|
| P0 全部 `verified` 或有明确 `waived` | ❌ 未满足 | 24/28 仍为 `blocked` |
...
| JVM 测试证据 | 28/28 (100%) | 71+ 核心数据层测试方法，400+ 总测试，0 失败 |
| 契约映射 | 28/28 (100%) | 所有 P0 入口均有原生映射（部分 deferred/reduced） |
